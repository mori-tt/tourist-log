...
export default function ArticleDetailPage() {
  ...
  const submitTip = async () => {
    ...
    try {
      ...
      // 受取人のウォレットアドレスを取得
      const recipientRes = await fetch(`/api/user/${article.userId}`);
      const recipientData = await recipientRes.json();

      if (!recipientData.walletAddress) {
        setError("著者のウォレットアドレスが設定されていません。");
        return;
      }
      ...
      const response = await fetch("/api/transactions/tip", {
        ...
        body: JSON.stringify({
          ...
          recipientAddress: recipientData.walletAddress,
      ...
      if (response.ok) {
        ...
        alert(
          "投げ銭が完了しました！トランザクションハッシュ: " +
            result.blockchain.transactionInfo.hash
        );
      } else {
        const errorData = await response.json();
        setError(errorData.error || "投げ銭処理に失敗しました。");
      }
    } catch (error) {
      console.error("投げ銭処理エラー:", error);
      setError("投げ銭処理に失敗しました。");
    } finally {
      ...
      <Dialog open={tipDialogOpen} onOpenChange={setTipDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button type="submit" onClick={submitTip} disabled={isSubmitting}>
              {isSubmitting ? "処理中..." : "送信"}
            </Button>
          </DialogFooter>
        </DialogContent>
